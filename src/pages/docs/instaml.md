...
```javascript
transact([
  tx.goals[id()].update({title: ["eat", "sleep", "hack", "repeat"][Math.floor(Math.random() * 4)]})
]);
```
...
```javascript
transact([
  tx.goals["eat"].update({lastTimeEaten: "Today"})
]);
```
...
```javascript
transact([
  tx.goals["health"].link({todos: "workout"})
])
```
...
```javascript
transact([
  tx.goals["health"].link({priorities: "workout"})
])
```
...
```javascript
transact([
  tx.goals["health"].unlink({todos: "workout"})
]);
```
...
```javascript
transact([
  tx.todos["workout"].unlink({_goals: "health"})
]);
```
